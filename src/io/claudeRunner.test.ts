...
import { createSessionFolder } from './claudeRunner'
...
test('createSessionFolder copies the template and creates the rest of the scaffold', async () => {
  ...
  const metadata = JSON.parse(await readFile(path.join(sessionTmpDir, 'metadata.json'), 'utf-8'))
  ...
  const usage = JSON.parse(await readFile(path.join(sessionTmpDir, 'usage.json'), 'utf-8'))
