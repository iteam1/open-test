...
afterEach(async () => {
  ...
  if (fragmentsTmpDir) await rm(fragmentsTmpDir, { recursive: true, force: true })
})

async function freshSession(): Promise<{ sessionDir: string; sessionId: string }> {
...
test(
  '5.1: a turn runs fine with the fragment server attached (no regression)',
  async () => {
...
test(
  '5.9/5.10: save_fragment then run_fragment across two turns — tools reachable, usedFragmentTool set, reuse cheaper',
  async () => {
    const { sessionDir, sessionId } = await freshSession()
    ...
    const files = (await readdir(fragmentsTmpDir)).filter((f) => f.endsWith('.md'))
