...
- Able to manage working sessions in a central place, `./sessions/`, with each session kept as a separate folder. Sessions can be force-killed or resumed, etc. Each session stores test artifacts, a `metadata.json` file (just `session_id` — stable, defaults to a year-month-day-time format, never itself renamed —, `claude_session_id`, `created_at`), and a `usage.json` file (a list of usage entries, one per turn). The display name shown to the user *is* renamable, but that's a separate thing layered on top, not the `session_id` itself
